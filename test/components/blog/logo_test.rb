...
class Blog::Logo::ComponentTest < ViewComponent::TestCase
  def test_renders
  ...
  def test_sets_blog_name
    component = build_component(name: "Hello World")
    render_inline(component)
    assert_selector "a", text: "Hello World"
  end

  def test_sets_adds_an_svg
    logo = <<~SVG.strip
    ...
    SVG
    component = build_component(logo:)
    render_inline(component)
    assert_selector "svg.lucide-banana"
  end

  private

  def build_component(**)
    Blog::Logo::Component.new(**)
  end
end
